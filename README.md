...
revalidation, and key-change paths that could call the missing-license API. The current
v1.5 build also enables the four game/tool cards and returns `true` directly from the
async feature-level `verifyAccess` gate, preventing the secondary “key verification
failed” message when a card is opened. It strips signing material and uploads
`Ios_Store_VIP_v1.5_verify-access-always-true_unsigned.ipa`. The patcher verifies exact
source/output instructions, bundle version, ZIP integrity, `cryptid=0`, and the patched
executable SHA-256 before upload.
